...
| F21 | Modo no-interactivo | Flag `--no-interactive` + helper `isInteractive()` que respeta la env `CI` y `stdin.isTTY`. Habilita que agentes de IA y pipelines usen el CLI sin colgarse en prompts. Patrón tomado de OpenSpec — ver [docs/cli-best-practices.md](docs/cli-best-practices.md) | 🟠 | — |
| F22 | `--version` desde `package.json` | Quitar el `'0.1.0'` hardcodeado en `src/cmd/soia.ts`; leerlo de `package.json` (una sola fuente de verdad, sin drift por release) | 🟡 | — |
| F23 | `--no-color` global + hooks de ciclo de vida | Opción global `--no-color` (respeta el estándar `NO_COLOR`) y hooks `preAction`/`postAction` de Commander para cross-cutting (color, warnings de deprecación) | 🟡 | — |
...
| F13 | Contrato de salida `--json` | Salida machine-readable determinista en comandos de lectura (`models`, `fallback`, y futuros `list`/`status`) para consumo por agentes/tooling. Ver [docs/cli-best-practices.md](docs/cli-best-practices.md) | ⚪ | — |
