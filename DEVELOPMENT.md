...
npm start -- <cmd>       # Ejecutar la versión compilada (node dist/cmd/swarm.js)
...
- `src/` — código del CLI (`cmd/swarm.ts` + `lib/`).
...
- **Symlinks en modo global (en remoción)**: la implementación actual de `swarm init -g` usa symlinks
